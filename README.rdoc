...
try to issue your command up to five times. Important ones like add_server() and
remove_server() make very sure the command has worked and return true or false
accordingly.
...
=== Boolean (you can only set, but not get, these)
    ...
    pen.stubborn=true         # If the initial server selection is unavailable, close the 
     en                       # client connection without trying another
